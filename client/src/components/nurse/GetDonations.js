...
export default class GetDonations extends Component {
    constructor(props) {
        super(props);
        this.state = {
            mediInfo: [],
    ...
    getInfo() {
        axios.get("http://localhost:8000/mediInfoDonors").then(res => {
            if (res.data.success) {
                this.setState({
    ...
    columns = [
        {
            name: "Donor",
            selector: (row) => <Button style={{ backgroundColor: "#002D62", color: "white", borderRadius: "20px" }} size="sm" onClick={() => this.handleShowDonor(row.donorId)}>View</Button>
        },
        {
            name: "Date",
